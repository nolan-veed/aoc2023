s = """Game 1: 8 green, 4 red, 4 blue; 1 green, 6 red, 4 blue; 7 red, 4 green, 1 blue; 2 blue, 8 red, 8 green
...
#     valid = True
#     for round in game_parts[1].split(';'):
#         cube_parts = round.split(',')
#         for cube in cube_parts:
#             cube_desc = cube.strip().split(' ')
#             cube_count = int(cube_desc[0])
#             cube_color = cube_desc[1]
#             if cube_count > valid_game[cube_color]:
#                 valid = False
#                 break
#         if not valid:
#             break
...
for line in s.split('\n'):
    ...
    for round in game_parts[1].split(';'):
        ...
        for cube in cube_parts:
            ...
            cube_count = int(cube_desc[0])
            cube_color = cube_desc[1]
            if min_game[cube_color] == 0:
                min_game[cube_color] = cube_count
            elif min_game[cube_color] < cube_count:
                min_game[cube_color] = cube_count
