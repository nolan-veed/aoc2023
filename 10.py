...
s = """F|J-F|.|.F-F-7-7F--|77F.7.77F|-L7-|7|-|-F-L7.7FJF-7-JF7.77-F.7F7.FLF7-FF-L7F--F-J-..J-FJ-FLF7.FF.-F7-|-.F7--77F---7-LF7FFL7.F7|7FJ-FL-.-FJ-F
...
for s_row, line in enumerate(maze):
    found = False
    for s_col, c in enumerate(line):
        if c == 'S':
            found = True
            break
    if found:
...
# indices are clockwise, while going... up, right, bottom, right

dir_incrs = [(-1, 0), (0, 1), (1, 0), (0, -1)]
...
first_next_dirs = [('|', '7', 'F'), ('-', 'J', '7'), ('|', 'J', 'L'), ('-', 'F', 'L')]

dirs = []


def do_moves(next_dir):
    count_moves = 0
    c_row = s_row
    c_col = s_col
    first_time = True
    ...
    while True:
        incr_row, incr_col = dir_incrs[next_dir][0], dir_incrs[next_dir][1]
        n_row, n_col = c_row + incr_row, c_col + incr_col
        next_c = maze[n_row][n_col]

        if first_time and next_c not in first_next_dirs[next_dir]:
            break
        first_time = False
        ...
        if next_c == 'S':
            maze[n_row] = maze[n_row][:n_col] + '*' + maze[n_row][n_col + 1:]
            break
        next_dirs = next_dir_map[next_c]
        next_dir = next_dirs[next_dir]
        maze[n_row] = maze[n_row][:n_col] + '*' + maze[n_row][n_col + 1:]
...
def mark_tiles():
    ...
    for next_dir_i in range(len(dirs)):
        next_dir = dirs[next_dir_i]
        ...
        if going_up is not None:
            ...
            index = line1.rfind('*')
            if index == -1:
                line1_new = colors[0] * len(line1)
            else:
                line1_new = line1[:index + 1] + (colors[0] * (len(line1) - 1 - index))

            index = line2.find('*')
            if index == -1:
                line2_new = colors[1] * len(line2)
            else:
                line2_new = (colors[1] * index) + line2[index:]

            line_new = line1_new + '*' + line2_new
            maze[c_row] = line_new
